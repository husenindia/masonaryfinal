...
(function( $ ){
  ...
  $.fn.cstMasonryFn = function( options ) {  
	
    var settings = {
     totalColumns:4 // DEFUALT VALUE FOR THE TOTALCOLUMNS
    };
    ...
    return this.each(function() {        
      ...
      // with our default settings
		...
		//$cstMasonry=$('.mega-sub-menu-main');
		$cstMasonry=$(this);
		var totalColumn=settings.totalColumns;
		var columnDividerCounter=0;
		var arrayKeyCounter=0;
		var arrOfColumn=[0];		
		var liHeight=0;
		...
		$cstMasonry.find('> li').outerWidth(100/totalColumn+'%'); 
		$cstMasonry.find('> li').each(function(){			
			$this=$(this);			
			liHeight=$this.height();
			arrayKey=arrOfColumn.indexOf(Math.min.apply(Math, arrOfColumn)); // FIND WHICH COLUMN HAS SPACE IN COMPARISION TO OTHER COLUMN
		...
		// SETTING THE OUTER UL HEIGHT
		heightValue=Math.max.apply(Math, arrOfColumn); // FIND WHICH COLUMN HAS SPACE IN COMPARISION TO OTHER COLUMN
		$cstMasonry.innerHeight(heightValue);
    });
